...
The docker image is named `gisaia/swagger-codegen-3.0.25`, and its version is contained in file [docker_image_version](../docker_image_version)
...
```bash
swagger-codegen/3.0.25/build.bash
```

# Publication

```bash
SIMULATE=false swagger-codegen/3.0.25/publish.bash
```
