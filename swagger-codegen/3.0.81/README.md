...
The docker image is named `gisaia/swagger-codegen-3.0.81`, and its version is contained in file [docker_image_version](../docker_image_version)
...
```bash
swagger-codegen/3.0.81/build.bash
```

# Publication

```bash
SIMULATE=false swagger-codegen/3.0.81/publish.bash
```
