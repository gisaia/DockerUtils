...
The docker image is named `gisaia/swagger-codegen-2.2.3`, and its version is contained in file [docker_image_version](../docker_image_version)
...
```bash
swagger-codegen/2.2.3/build.bash
```

# Publication

```bash
SIMULATE=false swagger-codegen/2.2.3/publish.bash
```
...
# - Generate code, based on API description file `swagger.json` and configuration file `python-config.json`, and output it in directory `typescript-fetch`.
...
docker run \
  ...
  --mount dst=/input/config.json,src="$PWD/python-config.json",type=bind,ro \
  --mount dst=/output,src="$PWD/python-api",type=bind \
  --rm \
